...
DT_FORMAT = "%H:%M:%S";
MULTIPLIER = 2; LS_ITER = 1500 ;VNS_ITR = 2;NUM_TABLE = 24;
...
class Utils:
    ...
    def fetch(o=(18.509218,99.002598),d =(18.259722,99.490636),\
          mode = "time",t="08:00:00",isTolls = False):
    ...
    def get_data_from_cache(f:Vehicle or Stop,t : Vehicle or Stop,mode = "distance"):
        ...
        if(isinstance(f,Stop) and isinstance(t,Stop)):
            ...
            r1 = H[h]["s_cache"].loc(a,b)[f'{mode}'];
            r2 = H[(h+1)%len(H)]["s_cache"].loc(a,b)[f'{mode}'];
            return (r1+r2)//2
    ...
    def create_function(points = [()]):
        x,all_L,fn = symbols('x'),{},0;
        for n in range(len(points)):
            top = btm = 1;
            for i,p in enumerate(points):
                if(i == n): continue;
                top *= (x-points[i][0]);
                btm *= (points[n][0] - points[i][0]);
            all_L[n] = top/btm;
        terms_arr = [all_L[i]*points[i][1] for i in range(len(points))];
        for t in terms_arr: fn += t
        return fn;
    ...
    def generate_points_with_time(f,t,mode="distance"):
        ...
        if(isinstance(f,Stop) and isinstance(t,Stop)):
            ...
            h,m,s = [int(e) for e in Utils.s2d(f.time).split(":")];
            time = f"{h}.{m}";
            points = [(i,H[i]["s_cache"].loc(a,b)[f'{mode}']) for i in range(len(H))];
            ...
            for i in range(len(H)):
                hv = H[i]["v_cache"];
                points.append((i,Utils.vd(1,[lat_b,lng_b],hv,hv,mode)));
            ...
            for i in range(len(H)):
                hv = H[i]["v_cache"];
                points.append((i,Utils.vd(0,[lat_b,lng_b],hv,hv,mode)));
    ...
    @staticmethod
    def p(f:Vehicle or Stop,t : Vehicle or Stop,mode = "distance"):
        points,time = Utils.generate_points_with_time(f,t,mode);
        fn,x = Utils.create_function(points),symbols('x');
        return fn.subs(x,float(time));
    ...
    def p3(f:Vehicle or Stop,t : Vehicle or Stop,mode = "distance"):
        ...
        return interpolate.splev(float(time), tck)
