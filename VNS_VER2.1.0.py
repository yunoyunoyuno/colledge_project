...
DT_FORMAT = "%H:%M:%S";
MULTIPLIER = 2; LS_ITER = 600;VNS_ITR = 2;
...
class Utils:
    ...
    @staticmethod
    def generate_stops_cache(latlng,mode="time"):
        ...
        print("Finsihed Generated! Stops");

    @staticmethod
    def generate_vehicles_cache(latlng,mode="time"):
        for i in range(len(latlng)//400+1):
            r1 = Utils.fetch_mat([(18.509218,99.002598)],latlng[i*400:400*i+400],mode);
            if(r1["status"] == "OK"): htb_v[f"1 {i*400},{400*i+400}"] = r1["results"];
        for j in range(len(latlng)//8+1):
            r2 = Utils.fetch_mat(latlng[8*j:8*j+8],[(18.509218,99.002598)],mode);
            if(r2["status"] == "OK"): htb_v[f"0 {8*j},{8*j+8}"] = r2["results"]          
        print("Finsihed Generated! Vehicles");
    ...
    def vd(m,latlng):
        ...
        for keys in htb_v.keys():
            ...
            [d0,d1] = d.split(","); d0,d1 = int(d0),int(d1);
            if(m == 1): return htb_v[keys][0][index%400];
            elif(m == 0 and int(f) == 0 and (index in range (d0,d1))):
                index = (index - d0)%8; return htb_v[keys][index][0];
        else: return None

    @staticmethod
    def execute_by_Q(F,latlng,mode):
        queue = Q();
        for fn in F: queue.enQ(fn);
        while(queue.size() > 0): f = queue.deQ();f(latlng,mode);
    
    @staticmethod
    def get_data_from_cache(f:Vehicle or Stop,t : Vehicle or Stop):
    ...
    # select minimum start time
    @staticmethod 
    def arrive_time(v,s,now = 0):
        t =  Utils.time_btw_approx(v,s);
...
def cw(v : Vehicle,stops : list):
    ...
    while(h.size() > 0):
        ...
        if(s.vehicle == None):
            ...
            if(status == True):
                assigned_route.append({"stop" : s, "time" : time}); s.time = time;
...
start = T.time();
Utils.execute_by_Q(exe_fn,s_latlng,mode = "time");
t = T.time() - start ; print("Congrats! It's done !!! with table sizes",\
htb_s.size(),"data sizes",len(s_latlng)**2); print(t,"seconds",t/60,"minutes");
